...
public class ExecuteJobsRunnable implements Runnable {
  ...
  public void run() {
    ...
    ProcessEngineConfigurationImpl processEngineConfigurationImpl = processEngine.getProcessEngineConfiguration();
    commandExecutor = processEngineConfigurationImpl.getCommandExecutorTxRequired();
    boolean isMetricsEnabled = processEngineConfigurationImpl.isMetricsEnabled();
    ...
    try {
      while (!currentProcessorJobQueue.isEmpty()) {
        ...
        long startTime = ClockUtil.getCurrentTime().getTime();

        try {
          executeJob(nextJobId, commandExecutor);

          if(isMetricsEnabled) {
            processEngineConfigurationImpl.getJobExecutorMonitor()
              .jobExecuted(ClockUtil.getCurrentTime().getTime() - startTime, true);
          }
          ...
          if(isMetricsEnabled) {
            processEngineConfigurationImpl.getJobExecutorMonitor()
              .jobExecuted(ClockUtil.getCurrentTime().getTime() - startTime, false);
          }
        }
